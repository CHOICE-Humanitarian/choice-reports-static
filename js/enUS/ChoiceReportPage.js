...
async function putimportChoiceReport($formValues, pk, success, error) {
	var json = $formValues.find('.PUTImport_searchList').val();
	if(json != null && json !== '')
		putimportChoiceReportVals(JSON.parse(json), success, error);
}

function putimportChoiceReportVals(json, success, error) {
	$.ajax({
...
async function postChoiceReport($formValues, success, error) {
	var vals = {};
	if(success == null) {
	...
	var valueDonorKey = $formValues.find('.valueDonorKey').val();
	if(valueDonorKey != null && valueDonorKey !== '')
		vals['donorKey'] = valueDonorKey;
...
function postChoiceReportVals(vals, success, error) {
	$.ajax({
		url: '/api/report'
		, dataType: 'json'
		, type: 'POST'
...
async function patchChoiceReport($formFilters, $formValues, pk, success, error) {
	var filters = patchChoiceReportFilters($formFilters);
	...
	var valueDonorKey = $formValues.find('input.valueDonorKey:checked').val();
	if(valueDonorKey != null && valueDonorKey !== '')
		vals['setDonorKey'] = valueDonorKey;
	...
	patchChoiceReportVals(pk == null ? $.deparam(window.location.search ? window.location.search.substring(1) : window.location.search) : [{name:'fq', value:'pk:' + pk}], vals, success, error);
}

function patchChoiceReportFilters($formFilters) {
	var filters = [];
	if($formFilters) {
		...
		var filterDonorKey = $formFilters.find('.valueDonorKey').val();
		if(filterDonorKey != null && filterDonorKey !== '')
			filters.push({ name: 'fq', value: 'donorKey:' + filterDonorKey });
...
function patchChoiceReportVal(filters, v, val, success, error) {
	var vals = {};
	vals[v] = val;
	patchChoiceReportVals(filters, vals, success, error);
}

function patchChoiceReportVals(filters, vals, success, error) {
	$.ajax({
		url: '/api/report?' + $.param(filters)
		, dataType: 'json'
		, type: 'PATCH'
		, contentType: 'application/json; charset=utf-8'
		, data: JSON.stringify(vals)
...
async function getChoiceReport(pk) {
	$.ajax({
		url: '/api/report/' + id
		, dataType: 'json'
		, type: 'GET'
...
async function searchChoiceReport($formFilters, success, error) {
	var filters = searchChoiceReportFilters($formFilters);
	if(success == null)
		success = function( data, textStatus, jQxhr ) {};
	if(error == null)
		error = function( jqXhr, textStatus, errorThrown ) {};
...
function searchChoiceReportFilters($formFilters) {
	var filters = [];
	if($formFilters) {
...
function searchChoiceReportVals(filters, success, error) {
...
function suggestChoiceReportObjectSuggest($formFilters, $list) {
	success = function( data, textStatus, jQxhr ) {
		$list.empty();
		$.each(data['list'], function(i, o) {
			var $i = $('<i>').attr('class', 'fad fa-file-chart-line ');
	...
	error = function( jqXhr, textStatus, errorThrown ) {};
	searchChoiceReportVals($formFilters, success, error);
}

function suggestChoiceReportDonorKey(filters, $list, pk = null, relate=true) {
	success = function( data, textStatus, jQxhr ) {
		$list.empty();
		$.each(data['list'], function(i, o) {
			var $i = $('<i>').attr('class', 'fa fa-hands-heart ');
			var $span = $('<span>').attr('class', '').text(o['objectTitle']);
			...
			var $input = $('<input>');
			$input.attr('id', 'GET_donorKey_' + pk + '_reportKeys_' + o['pk']);
			$input.attr('value', o['pk']);
			$input.attr('class', 'valueDonorKey w3-check ');
			if(pk != null) {
				$input.attr('onchange', "var $input = $('#GET_donorKey_" + pk + "_reportKeys_" + o['pk'] + "'); patchChoiceReportVals([{ name: 'fq', value: 'pk:" + pk + "' }], { [($input.prop('checked') ? 'set' : 'remove') + 'DonorKey']: \"" + o['pk'] + "\" } ); ");
		...
		var focusId = $('#ChoiceReportForm :input[name="focusId"]').val();
		if(focusId)
...
async function websocketChoiceReport(success) {
	window.eventBus.onopen = function () {
		...
		window.eventBus.registerHandler('websocketChoiceReport', function (error, message) {
			...
			var id = json['id'];
			var pk = json['pk'];
			var pkPage = $('#ChoiceReportForm :input[name=pk]').val();
			var pks = json['pks'];
			...
			var $header = $('<div>').attr('class', 'w3-container fa-cyan ').attr('id', 'header-' + id);
			var $i = $('<i>').attr('class', 'fad fa-file-chart-line w3-margin-right ').attr('id', 'icon-' + id);
			var $headerSpan = $('<span>').attr('class', '').text('modify reports in ' + json.timeRemaining);
			var $x = $('<span>').attr('class', 'w3-button w3-display-topright ').attr('onclick', '$("#card-' + id + '").hide(); ').attr('id', 'x-' + id);
			var $body = $('<div>').attr('class', 'w3-container w3-padding ').attr('id', 'text-' + id);
			var $bar = $('<div>').attr('class', 'w3-light-gray ').attr('id', 'bar-' + id);
			var $progress = $('<div>').attr('class', 'w3-cyan ').attr('style', 'height: 24px; width: ' + percent + '; ').attr('id', 'progress-' + id).text(numPATCH + '/' + numFound);
			$card.append($header);
			$header.append($i);
		...
		window.eventBus.registerHandler('websocketChoiceDonor', function (error, message) {
			$('#Page_donorKey').trigger('oninput');
			$('#Page_donorKey_add').text('add a donor');
			$('#Page_donorKey_add').removeClass('w3-disabled');
			$('#Page_donorKey_add').attr('disabled', false);
		});
	}
}
async function websocketChoiceReportInner(apiRequest) {
	...
	if(pk != null) {
		searchChoiceReportVals([ {name: 'fq', value: 'pk:' + pk} ], function( data, textStatus, jQxhr ) {
			var o = data['list'][0];
			var val = o['pk'];
			if(vars.includes('pk')) {
				$('.inputChoiceReport' + pk + 'Pk').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Pk').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Pk'));
			}
			var val = o['created'];
			if(vars.includes('created')) {
				$('.inputChoiceReport' + pk + 'Created').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Created').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Created'));
			}
			var val = o['modified'];
			if(vars.includes('modified')) {
				$('.inputChoiceReport' + pk + 'Modified').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Modified').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Modified'));
			}
			var val = o['objectId'];
			if(vars.includes('objectId')) {
				$('.inputChoiceReport' + pk + 'ObjectId').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ObjectId').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ObjectId'));
			}
			var val = o['archived'];
			if(vars.includes('archived')) {
				$('.inputChoiceReport' + pk + 'Archived').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Archived').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Archived'));
			}
			var val = o['deleted'];
			if(vars.includes('deleted')) {
				$('.inputChoiceReport' + pk + 'Deleted').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Deleted').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Deleted'));
			}
			var val = o['donorKey'];
			if(vars.includes('donorKey')) {
				$('.inputChoiceReport' + pk + 'DonorKey').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorKey').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorKey'));
			}
			var val = o['donorFullName'];
			if(vars.includes('donorFullName')) {
				$('.inputChoiceReport' + pk + 'DonorFullName').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorFullName').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorFullName'));
			}
			var val = o['donorParentName'];
			if(vars.includes('donorParentName')) {
				$('.inputChoiceReport' + pk + 'DonorParentName').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorParentName').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorParentName'));
			}
			var val = o['donorId'];
			if(vars.includes('donorId')) {
				$('.inputChoiceReport' + pk + 'DonorId').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorId').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorId'));
			}
			var val = o['donorAttributeId'];
			if(vars.includes('donorAttributeId')) {
				$('.inputChoiceReport' + pk + 'DonorAttributeId').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorAttributeId').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorAttributeId'));
			}
			var val = o['donorInKind'];
			if(vars.includes('donorInKind')) {
				$('.inputChoiceReport' + pk + 'DonorInKind').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorInKind').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorInKind'));
			}
			var val = o['donorTotal'];
			if(vars.includes('donorTotal')) {
				$('.inputChoiceReport' + pk + 'DonorTotal').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorTotal').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorTotal'));
			}
			var val = o['donorYtd'];
			if(vars.includes('donorYtd')) {
				$('.inputChoiceReport' + pk + 'DonorYtd').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorYtd').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorYtd'));
			}
			var val = o['donorQ1'];
			if(vars.includes('donorQ1')) {
				$('.inputChoiceReport' + pk + 'DonorQ1').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorQ1').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorQ1'));
			}
			var val = o['donorQ2'];
			if(vars.includes('donorQ2')) {
				$('.inputChoiceReport' + pk + 'DonorQ2').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorQ2').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorQ2'));
			}
			var val = o['donorQ3'];
			if(vars.includes('donorQ3')) {
				$('.inputChoiceReport' + pk + 'DonorQ3').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorQ3').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorQ3'));
			}
			var val = o['donorQ4'];
			if(vars.includes('donorQ4')) {
				$('.inputChoiceReport' + pk + 'DonorQ4').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorQ4').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorQ4'));
			}
			var val = o['donorLogoFilename'];
			if(vars.includes('donorLogoFilename')) {
				$('.inputChoiceReport' + pk + 'DonorLogoFilename').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'DonorLogoFilename').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'DonorLogoFilename'));
			}
			var val = o['inheritPk'];
			if(vars.includes('inheritPk')) {
				$('.inputChoiceReport' + pk + 'InheritPk').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'InheritPk').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'InheritPk'));
			}
			var val = o['classCanonicalName'];
			if(vars.includes('classCanonicalName')) {
				$('.inputChoiceReport' + pk + 'ClassCanonicalName').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ClassCanonicalName').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ClassCanonicalName'));
			}
			var val = o['classSimpleName'];
			if(vars.includes('classSimpleName')) {
				$('.inputChoiceReport' + pk + 'ClassSimpleName').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ClassSimpleName').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ClassSimpleName'));
			}
			var val = o['classCanonicalNames'];
			if(vars.includes('classCanonicalNames')) {
				$('.inputChoiceReport' + pk + 'ClassCanonicalNames').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ClassCanonicalNames').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ClassCanonicalNames'));
			}
			var val = o['sessionId'];
			if(vars.includes('sessionId')) {
				$('.inputChoiceReport' + pk + 'SessionId').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'SessionId').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'SessionId'));
			}
			var val = o['userKey'];
			if(vars.includes('userKey')) {
				$('.inputChoiceReport' + pk + 'UserKey').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'UserKey').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'UserKey'));
			}
			var val = o['saves'];
			if(vars.includes('saves')) {
				$('.inputChoiceReport' + pk + 'Saves').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Saves').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Saves'));
			}
			var val = o['objectTitle'];
			if(vars.includes('objectTitle')) {
				$('.inputChoiceReport' + pk + 'ObjectTitle').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ObjectTitle').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ObjectTitle'));
			}
			var val = o['objectSuggest'];
			if(vars.includes('objectSuggest')) {
				$('.inputChoiceReport' + pk + 'ObjectSuggest').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ObjectSuggest').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ObjectSuggest'));
			}
			var val = o['objectText'];
			if(vars.includes('objectText')) {
				$('.inputChoiceReport' + pk + 'ObjectText').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'ObjectText').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'ObjectText'));
			}
			var val = o['pageUrlId'];
			if(vars.includes('pageUrlId')) {
				$('.inputChoiceReport' + pk + 'PageUrlId').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'PageUrlId').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'PageUrlId'));
			}
			var val = o['pageUrlPk'];
			if(vars.includes('pageUrlPk')) {
				$('.inputChoiceReport' + pk + 'PageUrlPk').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'PageUrlPk').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'PageUrlPk'));
			}
			var val = o['pageUrlApi'];
			if(vars.includes('pageUrlApi')) {
				$('.inputChoiceReport' + pk + 'PageUrlApi').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'PageUrlApi').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'PageUrlApi'));
			}
			var val = o['id'];
			if(vars.includes('id')) {
				$('.inputChoiceReport' + pk + 'Id').each(function() {
					if(val !== $(this).val())
						$(this).val(val);
				});
				$('.varChoiceReport' + pk + 'Id').each(function() {
					if(val !== $(this).text())
						$(this).text(val);
				});
				addGlow($('.inputChoiceReport' + pk + 'Id'));
			}
		});
	}
}
